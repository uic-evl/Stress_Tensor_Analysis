...
char *volumeFilename = "divergence.raw";
//char *volumeFilename = "pressure.raw";
char *cluster_file = "ds2_clust3_ushort.raw";
...
void render()
{
    ...
    cutilSafeCall(cudaMemset(d_cluster, 0, width*height*4));
		
    // call CUDA kernel, writing results to PBO
    render_kernel(gridSize, blockSize, d_output, d_cluster, d_iRed, d_oRed, d_iGreen, d_oGreen, d_iBlue, d_oBlue,
			d_iColors, data, cluster_data, width, height, density, brightness, transfer[0], transfer[1], transfer[2],
...
void reshape_cluster(int w, int h){
...
void display_cluster(){
    ...
    // display results
       ...
       if (g_CheckCluster && g_CheckCluster->IsQAReadback() && g_Verify) {
           // readback for QA testing
           shrLog("\n> (Frame %d) Readback BackBuffer\n", frameCount);
           g_CheckCluster->readback( width, height );
           g_CheckCluster->savePPM(sOriginal[g_Index], true, NULL);
           if (!g_CheckCluster->PPMvsPPM(sOriginal[g_Index], sReference[g_Index], MAX_EPSILON_ERROR, THRESHOLD)) {
               g_TotalErrors++;
           }
           g_Verify = false;
       }
...
int main( int argc, char** argv) 
{
	...
	h_volume = loadRawFile(path, size);   
	...
	initCuda(h_volume, volumeSize, transfer, width*height);
    ...
    if (g_bQAReadback) {
        ...
        g_CheckRender->EnableQAReadback(true);
		 ...
		 g_CheckCluster = new CheckBackBuffer(width, height, 4, false);
		...
		cutilSafeCall(cudaMalloc((void**)&d_cluster, width*height*sizeof(uint)));
        cutilSafeCall(cudaMemset(d_cluster, 0, width*height*sizeof(uint)));
		...
		cutilSafeCall( cudaMemcpy(g_CheckCluster->imageData(), d_cluster, width*height*4, cudaMemcpyDeviceToHost) );
        g_CheckRender->savePPM(sOriginal[g_Index], true, NULL);
        ...
        if (g_bQAGLVerify) {
			...
			g_CheckCluster = new CheckBackBuffer(width, height, 4);
            g_CheckCluster->setPixelFormat(GL_RGBA);
            g_CheckCluster->setExecPath(argv[0]);
            g_CheckCluster->EnableQAReadback(true);
        }
